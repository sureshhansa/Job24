...
function admin_job_form(?int $id): void
{
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
        ...
        // SLUG: POST se manually diya slug use karo (slugify karke)
        // Blank hai to: edit job = purana rakho, new job = title se generate
        $manualSlug = trim(post_param('slug'));
        if ($manualSlug !== '') {
            // Manual slug diya — slugify karke use karo
            $params['slug'] = unique_slug(slugify($manualSlug), 'jobs', $id ?: null);
        } elseif ($id) {
            // Edit job, slug blank — purana slug waise hi rakho
            $existing = fetch_one("SELECT slug FROM jobs WHERE id = ?", [$id]);
            $params['slug'] = $existing['slug'] ?? unique_slug(slugify($title), 'jobs', $id);
        } else {
            // New job, slug blank — title se auto generate
            $params['slug'] = unique_slug(slugify($title), 'jobs');
        }
